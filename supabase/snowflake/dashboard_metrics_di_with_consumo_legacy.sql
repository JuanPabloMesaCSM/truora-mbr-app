...
ip_actual AS (
  ...
  WHERE p.client_id = pe.client_id
    ...
    AND LOWER(p.status) IN ('success','failure')   -- excluye 'pending' y otros no-terminales
...
clasif_actual AS (
  SELECT
    ...
    CASE
      ...
      ELSE 0
    END AS es_expirado,
    -- ¿es error tecnico? (incluye system_error: variante real en SF que el filtro
    -- original no capturaba, dejaba ~177 procesos mal categorizados como "0")
    CASE
      WHEN LOWER(failure_status) IN ('technical_error','error','system_error') THEN 1
      ELSE 0
