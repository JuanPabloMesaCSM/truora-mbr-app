-- BotiAlertas v2 — BGC (Background Check)
-- Diferencias vs el query del flujo original:
--   * Whitelist dinamica desde Supabase (placeholder {{BGC_CLIENT_LIST_VALUES}}).
--     Reemplaza el NOT IN de 9 TCIs hardcodeados — ahora cualquier cliente fuera
--     del whitelist (incluyendo todos los tenants de prueba) queda excluido por
--     construccion. Tampoco depende de TRUORA_SCHEMA.CSM_CLIENTS (estaba stale).
--   * Sin LIMIT, sin filtro de variacion ni de volumen: la severidad se decide
--     en el Code node "classify".
--   * pct_change y avg_score se exportan como columnas crudas; el flujo n8n los
--     mete a metricas_extra.score_actual / score_anterior.
...
-- TENANT puede tener 2 filas por mismo truora_client_id (rebrand / duplicado)
-- — agregamos al nivel client_id para no multiplicar el resultado final.
tenant_one AS (
