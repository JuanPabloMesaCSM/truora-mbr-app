...
-- IMPORTANTE: excluimos type='document-validation' porque corre internamente
-- como sub-paso de DI, no es check BGC facturable. Sin este filtro inflaba
-- los KPIs de clientes que solo usan DI (caso real Enlace CSC: 93.922 checks
-- = 100% document-validation, sin checks BGC reales).
checks_actual AS (
  SELECT
    c.check_id,
    c.country,
  ...
  WHERE c.client_id = pe.client_id
    ...
    AND LOWER(c.type) != 'document-validation'
),
checks_prev AS (
  SELECT
    c.check_id,
