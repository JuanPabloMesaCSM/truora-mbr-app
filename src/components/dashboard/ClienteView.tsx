...
export default function ClienteView({
  ...
  // Counters facturables (CH) — fuente de verdad del header. Matchean lo que
  // el cliente ve en su front Truora y lo que se le cobra.
  const diBillable = parseTotalesBillable(data.data.DI);
  const bgcBillable = parseTotalesBillable(data.data.BGC);
  const ceBillable = parseTotalesBillable(data.data.CE);
    ...
    >
      ...
      {ranEjecutados.BGC && bgc && (
        <KpiCard
          ...
          breakdown={[
            { label: "Checks ejecutados", valor: fmtNum(bgc.totalChecks) },
