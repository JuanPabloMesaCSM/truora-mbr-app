...
export const MODULES: Record<Product, { base: ModuleDef; optional: ModuleDef[] }> = {
  DI: {
    ...
    optional: [
      { id: 'consumo_facturable', label: 'Consumo por Validador', desc: 'Barras horizontales + tabla', description: 'Validaciones por tipo (documento, rostro, TruFace, teléfono, email…) — coincide 1:1 con el consumo que el cliente ve en su consola Truora y con la factura. Tomado de ClickHouse. Es la unidad que el cliente sí puede cuadrar (distinta de "procesos iniciados").', chart: 'horizontal-bars' },
  ...
  BGC: {
    ...
    optional: [
      ...
      { id: '4b_score_matriz', label: 'Matriz Score × País', desc: 'Tabla pivote', description: 'Tabla detallada: cuántos checks hay por cada score (0-10) en cada país y qué % representa cada celda sobre el total de su país. Equivale al pivot de Excel que arman los clientes. Reusa la misma data de "Score por País".', chart: 'table' },
